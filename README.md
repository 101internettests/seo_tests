...
## Остановка при недоступности Google Sheets

Перед первым запросом к сайту анализатор проверяет чтение целевого листа из
`default_settings.spreadsheet_id` и `sheet_name`. Если подключение или чтение
не удалось, обход URL не начинается: в Telegram отправляется уведомление
«ОШИБКА АНАЛИЗА» с причиной и ссылкой на таблицу, процесс завершается с кодом `1`.
Jenkins отмечает такой запуск как упавший. Пустая доступная таблица не является ошибкой.

Если чтение перестало работать во время сравнения, оставшиеся URL не проверяются.
Ошибка записи результатов тоже завершает процесс с кодом `1`, без успешного
Telegram-отчета. Проверка перед обходом подтверждает чтение; права на запись
проверяются при сохранении результатов.

`--no-sheets` отключает загрузку, но чтение для сравнения по-прежнему обязательно.
`--no-telegram` отключает уведомления; ошибка Google Sheets все равно завершает
процесс с кодом `1`. `--list-sites` не требует подключения к Google.

Изолированные проверки этого поведения: `python -m pytest tests/test_sheets_fail_fast.py -q`.
