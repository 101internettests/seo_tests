...
В консоли Jenkins и файле лога ошибка выделена блоком «ПРОГОН ОСТАНОВЛЕН: GOOGLE SHEETS»:
этап и операция, HTTP-статус и тип ошибки, причина и подсказка для проверки,
подробности ответа, конфигурация, лист и ссылка, количество начатых и обработанных URL.
Отдельно фиксируются результат отправки Telegram-алерта и завершение с кодом `1`.
